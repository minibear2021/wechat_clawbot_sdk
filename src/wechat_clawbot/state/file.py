...
DEFAULT_STATE_DIR_ENV_VAR = "WECHAT_CLAWBOT_STATE_DIR"
LEGACY_STATE_DIR_ENV_VAR = "WECHATBOT_STATE_DIR"


def resolve_default_state_dir() -> Path:
    override = os.environ.get(DEFAULT_STATE_DIR_ENV_VAR)
    if not override:
        override = os.environ.get(LEGACY_STATE_DIR_ENV_VAR)
    if override:
        return Path(override).expanduser()
    if os.name == "nt":
        appdata = os.environ.get("APPDATA")
        if appdata:
            return Path(appdata) / "wechat-clawbot"
    if sys.platform == "darwin":
        return Path.home() / "Library" / "Application Support" / "wechat-clawbot"
    xdg_state_home = os.environ.get("XDG_STATE_HOME")
    if xdg_state_home:
        return Path(xdg_state_home).expanduser() / "wechat-clawbot"
    return Path.home() / ".local" / "state" / "wechat-clawbot"
...
class FileStateStore:
    ...
    async def load_poll_cursor(self, account_id: str) -> PollCursor:
        payload = await self._read_json_with_fallback(
            self._poll_cursor_path(account_id),
            self._legacy_poll_cursor_path(account_id),
        )
        if payload is None:
            return PollCursor()
    ...
    async def save_context_token(
        ...
        *,
        ...
        payload = await self._read_json_with_fallback(path, self._legacy_context_tokens_path(account_id)) or {}
        if context_token:
    ...
    async def load_context_token(self, *, account_id: str, user_id: str) -> str | None:
        payload = await self._read_json_with_fallback(
            ...
            self._legacy_context_tokens_path(account_id),
        )
        if payload is None:
            return None
    ...
    def _legacy_poll_cursor_path(self, account_id: str) -> Path:
        return self._root_dir / "poll_cursors" / f"{account_id}.json"

    def _legacy_context_tokens_path(self, account_id: str) -> Path:
        return self._root_dir / "context_tokens" / f"{account_id}.json"

    async def _read_json(self, path: Path) -> dict[str, object] | None:
        return await to_thread(self._read_json_sync, path)

    async def _read_json_with_fallback(self, primary: Path, fallback: Path) -> dict[str, object] | None:
        payload = await self._read_json(primary)
        if payload is not None:
            return payload
        if fallback == primary:
            return None
        return await self._read_json(fallback)
